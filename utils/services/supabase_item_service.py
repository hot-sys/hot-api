...
SUPABASE_BUCKET_ITEMS = os.getenv('SUPABASE_BUCKET_ITEMS')
...
def upload_single_image(file):
    ...
    try:
        ...
        file_name = f"Item_images/{uuid.uuid4().hex}_{file.name}"

        response = supabase.storage.from_(SUPABASE_BUCKET_ITEMS).upload(file_name, file.read())
        if hasattr(response, 'error') and response.error:
            raise Exception(response["error"]["message"])

        public_url = supabase.storage.from_(SUPABASE_BUCKET_ITEMS).get_public_url(file_name)
...
def remove_file(url):
    ...
    """
    try:
        bucket_url = f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET_ITEMS}/"
        ...
        response = supabase.storage.from_(SUPABASE_BUCKET_ITEMS).remove([file_path])
