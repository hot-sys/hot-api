...
SUPABASE_BUCKET_USER = os.getenv('SUPABASE_BUCKET_USER')
...
def upload_single_image(file):
    ...
    try:
        ...
        file_name = f"User_images/{uuid.uuid4().hex}_{file.name}"

        response = supabase.storage.from_(SUPABASE_BUCKET_USER).upload(file_name, file.read())
        if hasattr(response, 'error') and response.error:
            raise Exception(response["error"]["message"])

        public_url = supabase.storage.from_(SUPABASE_BUCKET_USER).get_public_url(file_name)
...
def remove_file(url):
    ...
    """
    try:
        bucket_url = f"{SUPABASE_URL}/storage/v1/object/public/{SUPABASE_BUCKET_USER}/"
        ...
        response = supabase.storage.from_(SUPABASE_BUCKET_USER).remove([file_path])
